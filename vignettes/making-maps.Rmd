...
A powerful feature of `gfwr` is the ability to get data for making custom maps of human activity at sea, such as apparent fishing effort, at-sea transshipment, and port visits. The `get_raster()` function provides gridded (e.g. raster) data from GFW's [4Wings Map Visualization API](https://globalfishingwatch.org/our-apis/documentation#map-visualization-4wings-api) and is useful for making heatmaps, while the `get_events()` function supplies vector data (mostly point locations) from the GFW [Events API](https://globalfishingwatch.org/our-apis/documentation#events-api) for individual vessel events. 

>This vignette demonstrates how to use and combine multiple `gfwr` functions to make a variety of maps of fishing vessel activity. Specifically, this vignette will show how to use the `get_raster()` function to make heatmaps of apparent fishing effort, and the `get_events()` function to visualize the locations of specific events. It will demonstrate how to request data for specific regions using `gfwr`'s built-in options, as well as how to use a custom region provided by the user.
...
```{r ita_plot, fig.width=10, fig.height=13}
eez_fish_all_df %>% 
...
```{r iotc_raster, fig.width=15, fig.height=10}
...
```{r iotc_plot_1, fig.width=13, fig.height=10}
iotc_p1 +
...
```{r plot_shp, fig.width=15, fig.height=7}
ggplot() +
...
Now we're ready to request data in our custom region from `get_raster()` and `get_events()`.
...
```{r loitering_ports_fishing_map, fig.height=14, fig.width=13}
my_raster_all_df %>% 
  filter(fishing_hours > 1) %>% 
  ggplot() +
  geom_raster(aes(x = Lon,
