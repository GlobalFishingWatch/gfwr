---
title: "Using `gfw_vessel_info()`: the basics of vessel identity in Global Fishing Watch and `gfwr`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using `gfw_vessel_info()`: the basics of vessel identity in Global Fishing Watch and `gfwr`}
...
This vignette explains the use of `gfw_vessel_info()` as a key function to 
...
the `gfw_vessel_info()` response and how to get __`vesselId`__ for use in the 
functions `gfw_event()` and `gfw_event_stats()`.
...
function `gfw_ais_fishing_hours()` in `gfwr`.
...
this is why simple calls to `gfw_vessel_info()` can return tables that have many
`vesselIds` and different identity markers in time.


## Basic and advanced searches

The function `gfw_vessel_info()` allows a user to run a basic query using MMSI, 
...
> **Note**:
> The same logic does not apply to `gfw_event()`: calls to `gfw_event()` using a 
