...
#' Wait for a bulk report to complete with exponential backoff
#'
#' @description
#' This is internal function that polls the [gfw_get_bulk_report_by_id()] until a bulk report status
#' reaches `"done"` or `"failed"`. It implements an exponential backoff strategy
#' to avoid overwhelming the API and to handle rate limits gracefully.
#'
#' **Important:**
#' We recommend to use this method to poll the status of previously created
#' bulk report, if it takes several minutes or hours to generate until it status
...
#' - A non-transient API error occurs.
#'
#' @details
#' For detailed information about the Get Bulk Report by ID API endpoint, please refer to the
#' official Global Fishing Watch API documentation:
...
#' `"adbb9b62-5c08-4142-82e0-b2b575f3e058"`.
#'
#' @param max_runtime Optional. Integer. Maximum time in seconds to wait for
#' the report. Defaults to `3600` (1 hour). Example: `3600`.
#'
#' @param initial_delay Optional. Integer. Initial delay between polls in seconds.
#' Defaults to `5`. Example: `5`.
#'
#' @param max_delay Optional. Integer. Maximum delay between polls in seconds.
#' Defaults to `300` (5 minutes). Example: `300`.
#'
#' @param key Character. API token. Defaults to [gfw_auth()].
#'
#' @param print_request Boolean. Whether to print the request, for debugging
...
#'
#' # Create a bulk report for SAR fixed infrastructure with oil structure filters in Argentina EEZ
#' report_created <- gfw_create_bulk_report(
#'   name = "sar-vessel-detection-gfwr-package-example-poll-wait-1",
#'   dataset = "public-fixed-infrastructure-data:latest",
#'   format = "CSV",
#'   region = list(
#'     dataset = "public-eez-areas",
...
#' )
#'
#' # Wait for the report to complete (poll until "done" or "failed")
#' report_waited <- gfw_wait_for_bulk_report(
#'   id = report_created$id[[1]],
#'   max_runtime = 1800, # Wait up to 30 minutes
#'   initial_delay = 10, # Start polling every 10 seconds
#'   print_request = TRUE
#' )
#'
#' # Access status to determine if report is ready for download or failed
#' print(report_waited$status[[1]])
#' }
#'
#' @keywords internal
#' @noRd
gfw_wait_for_bulk_report <- function(id = NULL,
                                     max_runtime = 3600L,
                                     initial_delay = 5L,
                                     max_delay = 300L,
                                     key = gfw_auth(),
  ...
  while (TRUE) {
    ...
    if (elapsed > max_runtime) {
      rlang::abort(glue::glue("Max runtime of {max_runtime}s exceeded while waiting for bulk report {id}."))
    }

    # Retrieve previously created bulk report metadata and status
    bulk_report <- gfw_get_bulk_report_by_id(id = id, key = key, print_request = print_request)
    status <- bulk_report$status[[1]]

    if (status %in% c("done", "failed")) {
      return(bulk_report)
    }

    # Still pending/processing: Sleep and Backoff
    Sys.sleep(delay)
...
#' Download bulk report file from a signed URL.
#'
#' @description
#' This is internal function that downloads a file (e.g. `"DATA"`, `"README"`, or `"GEOM"`)
#' from a signed URL generated by the Global Fishing Watch Bulk Reports API.
#'
#' Downloaded files are stored locally in a structured cache directory:
#' `data_dir/bulk-reports/<report_id>/`.
#'
#' @param url Required. Character. Signed HTTPS URL to the file.
#'
#' @param data_dir Optional. Character. Directory where downloaded files should be stored.
#' Defaults to `GFW_DATA_DIR` or a package-specific cache directory.
#'
#' @param overwrite Logical. If `TRUE`, existing downloaded file will be replaced.
#'
#' @param key Character. API token. Defaults to [gfw_auth()].
#'
#' @param print_request Boolean. Whether to print the request, for debugging
...
#'
#'
#' @return A character string (path) to the downloaded file, invisibly.
#'
#' @examples
#' \dontrun{
#' library(gfwr)
...
#'   print_request = TRUE
#' )
#' data_file_path <- gfw_download_bulk_report_file(url = data_url$url[[1]])
#'
#' # Download README documentation file
#' readme_url <- gfw_get_bulk_report_file_download_url(
#'   id = report_id,
#'   file = "README",
#'   print_request = TRUE
#' )
#' readme_file_path <- gfw_download_bulk_report_file(url = readme_url$url[[1]])
#' }
#'
#' @keywords internal
#' @noRd
gfw_download_bulk_report_file <- function(url = NULL,
                                          data_dir = NULL,
                                          overwrite = FALSE,
                                          key = gfw_auth(),
                                          print_request = FALSE) {
  ...
  if (!rlang::is_string(url) || !nzchar(url)) {
    cli::cli_abort(
      "{.arg url} is required and must be a non-empty character string."
    )
  }

  url <- trimws(url)
  url_parsed <- tryCatch(httr2::url_parse(url), error = function(e) NULL)
  is_valid_url_scheme <- rlang::is_string(url_parsed$scheme) && url_parsed$scheme %in% c("http", "https")
  is_valid_url_hostname <- rlang::is_string(url_parsed$hostname) && nzchar(url_parsed$hostname)
  if (!is_valid_url_scheme || !is_valid_url_hostname) {
    cli::cli_abort(
      "{.arg url} must be a valid HTTP or HTTPS URL."
    )
  }

  # Parse and extract bulk report id and file name
  url_path_parts <- strsplit(url_parsed$path, "/")[[1]]
  url_path_parts <- url_path_parts[url_path_parts != ""]
  if (length(url_path_parts) < 2) {
    cli::cli_abort(
      "{.arg url} path structure must follow {.path /report_id/file_name.ext} pattern."
    )
  }
  report_id <- url_path_parts[length(url_path_parts) - 1]
  report_file_name <- url_path_parts[length(url_path_parts)]

  ## Validate key -------------------------------------------------------------
  if (!rlang::is_string(key) || !nzchar(key)) {
    cli::cli_abort("No API token found. Set `GFW_TOKEN` or pass `key`.")
  }

  ## Resolve and create data directories --------------------------------------

  data_dir <- data_dir %||% Sys.getenv("GFW_DATA_DIR", unset = "")
  if (!rlang::is_string(data_dir) || !nzchar(trimws(data_dir))) {
    data_dir <- tools::R_user_dir("gfwr", which = "cache")
  }

  bulk_reports_dir <- file.path(data_dir, "bulk-reports")
  report_dir <- file.path(bulk_reports_dir, report_id)

  if (!dir.exists(report_dir)) {
    dir.create(report_dir, recursive = TRUE, showWarnings = FALSE)
  }

  file_path <- file.path(report_dir, report_file_name)

  if (rlang::is_true(overwrite) && file.exists(file_path)) {
  ...
  # Perform file stream download request --------------------------------------
  cli::cli_process_start("Downloading report {.val {report_id}} -> {.file {report_file_name}}.")
  ...
  tryCatch(
    {
      ...
      cli::cli_process_done(
        msg_done = paste0(
          "Download {.file {report_file_name}} for report {.val {report_id}} successfully."
        )
      )
    ...
    error = function(e) {
      ...
      cli::cli_abort(
        "Download {.file {report_file_name}} for report {.val {report_id}} failed."
      )
    }
  )
}
