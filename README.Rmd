...
this token to your `.Renviron` file (using `usethis::edit_r_environ()`) by adding
...
`where` for advanced searchers using SQL.
  ...
  + `where` search allows for the use of fuzzy matching with terms such as LIKE.
  + `includes` adds new information regarding the vessels. Options are "MATCH_CRITERIA",
...
You can use `get_vessel_info()` function first to extract `vessel id` from the `selfReportedInfo` list and then use it in the `get_event()` function:

```{r example_id_event, eval = TRUE}
vessel_id <- get_vessel_info(query = 224224000, key = key)
id <- vessel_id$selfReportedInfo$id
```
